...
import { dayjs, getLocalDate } from '@/utils/datetime';
import useCalendarDate from '@/hooks/useCalendarDate';

function MonthCell({ name, weeksCount, isLast }) {
  return (
    <div className={`col-span-${weeksCount} ${isLast && 'border-r-gray-900'} border-r flex items-center justify-center text-gray-900 p-1`}>
      {name}
    </div>
  );
};

function MonthsRow({ months = [] }) {
  return (
    <div className="grid grid-flow-col auto-cols-fr border-t border-gray-200 w-full">
      <div className="col-span-2 border-x"/>
      {months.map((month) => (
        <MonthCell
          key={month.index}
          name={month.name}
          weeksCount={month.weeks.length}
          isLast={!month.isLast}
        />
      ))}
    </div>
...
function WeekCell({ name, isLast, isCurrent }) {
  return (
    <div className={`${isLast && 'border-r-gray-900'} ${isCurrent && 'bg-red-200'} border-r border-t flex items-center justify-center text-gray-900 p-1`}>
      {name}
    </div>
  );
};

function WeeksRow({ months = [] }) {
  return (
    <div className="grid grid-flow-col auto-cols-fr border-gray-200 w-full">
      <div className="col-span-2 border-x" />
      {months.map((month) => month.weeks.map((week, wIndex) => (
        <WeekCell
          key={wIndex}
          name={week.nw}
          isLast={!month.isLast && week.isLast}
...
function TaskCell({ isLast, isFilled }) {
  return (
    <div className={`${isFilled && 'bg-blue-300'} ${isLast && 'border-r-gray-900'} ${isFilled && !isLast && 'border-r-0'} border-r p-1`} />
  );
};

function TaskRow({ task = {}, months = [] }) {
  return (
    <div className="grid grid-flow-col auto-cols-fr border-t border-gray-200 w-full">
      <div className="col-span-2 h-15 p-2 border-x flex justify-center flex-col">
        <div className="font-semibold text-gray-900">{task.title}</div>
        <div className="text-sm text-gray-500">{getLocalDate(task.startDate)} - {getLocalDate(task.endDate)}</div>
      </div>
      {months.map((month) => month.weeks.map((week, index) => (
        <TaskCell
          key={index}
          isLast={week.isLast && !month.isLast}
          isFilled={week.isDateWeek(task.startDate, task.endDate)}
        />
      )))}
    </div>
  );
...
function Calendar({ tasks, addTask, canAddTask }) {
  const {
    currentYear,
    currentQuarter,
  ...
  const currentQuarterTasks = useMemo(() => {
    const weeks = currentQuarterMonths.flatMap((month) => month.weeks);
  ...
  return (
    <>
      <section className="relative py-10">
        <div className="w-full mx-auto px-5 overflow-x-auto">
          <div className="flex flex-col md:flex-row max-md:gap-3 items-center justify-between mb-5">
            <div className="flex items-center gap-4">
              ...
              <h6 className="text-xl leading-8 font-semibold text-gray-900">{currentQuarter.name} {currentYear}</h6>
              <button onClick={() => setPrevQuarter()} className="text-indigo-600 p-1 rounded transition-all duration-300 hover:text-white hover:bg-indigo-600">
              ...
              <button onClick={() => setToday()} className="flex p-2 rounded-md border border-gray-200 items-center text-xs font-medium text-gray-900 transition-all duration-300 hover:bg-gray-100">
                Today
              </button>
              <button onClick={() => setNextQuarter()} className="text-indigo-600 p-1 rounded transition-all duration-300 hover:text-white hover:bg-indigo-600">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            <button onClick={() => canAddTask && addTask()} disabled={!canAddTask} className="py-2.5 px-5 bg-indigo-600 rounded-xl flex items-center gap-2 text-base font-semibold text-white transition-all duration-300 hover:bg-indigo-700 disabled:bg-indigo-300 disabled:cursor-not-allowed">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M10 5V15M15 10H5" stroke="white" strokeWidth="1.6" strokeLinecap="round"></path>
              </svg>
              New Task
            </button>
          </div>
          ...
          <div className="relative">
            <MonthsRow months={currentQuarterMonths} />

            <WeeksRow months={currentQuarterMonths} />

            {currentQuarterTasks.map((task, index) => (
              <TaskRow
                key={index}
                task={task}
            ...
            {!currentQuarterTasks.length && (
              <div className="w-full p-4 text-center border border-t-0 border-gray-200 text-2xl text-gray-900">
                No tasks for this quarter
              </div>
            )}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-2xl">
        <ul role="list" className="divide-y divide-gray-100 px-5 pb-5">
          {tasks.map((task, index) => (
            <li key={index} className="flex justify-between gap-x-6 py-3">
              <div className="flex min-w-0 gap-x-4">
                <div className="min-w-0 flex-auto">
                  <p className="text-sm font-semibold text-gray-900">{task.title}</p>
                  <p className="mt-1 truncate text-xs/5 text-gray-500">{getLocalDate(task.startDate)} - {getLocalDate(task.endDate)}</p>
                </div>
              </div>
              <div className='flex items-center'>
                <button onClick={() => setDate(task.startDate)} className="flex py-2 px-4 rounded-md border border-gray-200 items-center text-sm text-gray-900 transition-all duration-300 hover:bg-gray-100">
                  View
                </button>
              </div>
            </li>
